...
function generateWindowsManifest() {
  ...
  // electron-builder (NSIS) generates latest.yml automatically — check if it exists
  const autoGenerated = findFile(OUT_DIR, /^latest\.yml$/);
  if (autoGenerated) {
    console.log(`✓ Windows manifest already generated by electron-builder: ${autoGenerated}`);
    return;
  }

  // Fallback: find the NSIS Setup .exe in the output directory
  const exeFile = findFile(OUT_DIR, /Setup\.exe$/);
