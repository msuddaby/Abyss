...
public class ChatHub : Hub
{
    ...
    private async Task SendCurrentVoiceChannelStateToCaller(Guid channelGuid, string channelId)
    {
        ...
        if (watchParty != null)
        {
            var wpDto = new WatchPartyDto(
                ...
                watchParty.ProviderType, watchParty.PlaybackUrl);
        ...
        foreach (var idStr in attachmentIds)
        {
            if (Guid.TryParse(idStr, out var attId))
            {
                ...
                if (att != null)
                {
                    ...
                    attachments.Add(new AttachmentDto(att.Id, message.Id, att.FileName, att.FilePath, att.PosterPath, att.ContentType, att.Size, att.Width, att.Height));
                }
            }
        }
    ...
    public async Task PinMessage(string messageId)
    {
        ...
        var messageDto = new MessageDto(
            ...
            message.ChannelId,
            message.CreatedAt,
            message.Attachments.Select(a => new AttachmentDto(a.Id, a.MessageId!.Value, a.FileName, a.FilePath, a.PosterPath, a.ContentType, a.Size, a.Width, a.Height)).ToList(),
            message.EditedAt,
            message.IsDeleted,
            message.IsSystem,
