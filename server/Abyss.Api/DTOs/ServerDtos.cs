...
public record MessageDto(Guid Id, string Content, string AuthorId, UserDto Author, Guid ChannelId, DateTime CreatedAt, List<AttachmentDto> Attachments, DateTime? EditedAt, bool IsDeleted, List<ReactionDto> Reactions);
...
public record CreateRoleRequest(string Name, string Color, long Permissions);
public record UpdateRoleRequest(string? Name, string? Color, long? Permissions, int? Position);
