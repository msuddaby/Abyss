...
public record InviteDto(Guid Id, string Code, Guid? ServerId, string? CreatorId, DateTime CreatedAt, DateTime? ExpiresAt, int? MaxUses, int Uses, DateTime? LastUsedAt);
public record CreateServerInviteRequest(DateTime? ExpiresAt, int? MaxUses);
